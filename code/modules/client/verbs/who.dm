...
/client/verb/adminwho()
	...
	if(holder)
		...
		for(var/client/C in GLOB.admins)
			...
			if(!C.holder.fakekey)
				msg += "\t[C] is a [C.holder.rank]\n"
		msg += "<span class='info'>Adminhelps are also sent to discord. If no admins are available in game adminhelps will be relayed to admins in discord so that they can respond.</span>"
	to_chat(src, msg)
