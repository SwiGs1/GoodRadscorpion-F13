...
/mob/living/simple_animal/attack_hand(mob/living/carbon/human/M)
	...
	switch(M.a_intent)
		...
		if("harm", "disarm")
			...
			"<span class='boldannounce'>[M] [response_harm] [src]!</span>", null, COMBAT_MESSAGE_RANGE)
...
/mob/living/simple_animal/attack_hulk(mob/living/carbon/human/user, does_attack_animation = 0)
	if(user.a_intent == INTENT_HARM)
		...
		visible_message("<span class='danger'>[user] has punched [src]!</span>", \
			"<span class='boldannounce'>[user] has punched [src]!</span>", null, COMBAT_MESSAGE_RANGE)
		adjustBruteLoss(15)
		return TRUE
	...
	if(..()) //if harm or disarm intent.
		if(M.a_intent == INTENT_DISARM)
			...
			visible_message("<span class='danger'>[M] [response_disarm] [name]!</span>", \
					"<span class='boldannounce'>[M] [response_disarm] [name]!</span>", null, COMBAT_MESSAGE_RANGE)
			add_logs(M, src, "disarmed")
		else
			var/damage = rand(15, 30)
			visible_message("<span class='danger'>[M] has slashed at [src]!</span>", \
					"<span class='boldannounce'>[M] has slashed at [src]!</span>", null, COMBAT_MESSAGE_RANGE)
